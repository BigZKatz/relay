  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6">
        ...
        {/* Outbound bubble — show first recipient's interpolated version as preview */}
        {(() => {
            ...
            {visibleRecipients.map((r, i) => {
              ...
              return (
                <div key={i} className="space-y-1">
                  ...
                  {/* Show their personalized message */}
                  {personalizedBody && (
...
export default function InboxLayout({ messages }: { messages: Message[] }) {
  const router = useRouter();
  const [selected, setSelected] = useState<string | null>(
    messages[0]?.id ?? null
  );
  const [search, setSearch] = useState("");
  const [tab, setTab] = useState<"all" | "community" | "individual">("all");

  const filtered = messages.filter((m) => {
    const matchesTab =
      tab === "all" ||
      (tab === "community" && m.mode === "community") ||
      (tab === "individual" && m.mode !== "community");
    const matchesSearch =
      !search ||
  ...
  const activeMessageId = selectedMsg?.id ?? null;
    ...
    <div className="flex overflow-hidden bg-white h-[calc(100vh-56px)] lg:h-screen">
      ...
      <div className="w-72 flex-shrink-0 flex flex-col border-r border-slate-200 bg-white">
        ...
        <div className="flex border-b border-slate-100 px-2 pt-2">
          {(["all", "community", "individual"] as const).map((t) => (
            <button
              ...
              onClick={() => setTab(t)}
              className={cn(
                "flex-1 py-2 text-xs font-semibold capitalize rounded-t transition-colors",
                tab === t
                  ? "text-blue-600 border-b-2 border-blue-600"
                  : "text-slate-400 hover:text-slate-600"
              )}
            >
              {t === "community" ? "Blasts" : t === "individual" ? "1-on-1" : "All"}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="px-4 py-8 text-center text-sm text-slate-400">No messages found</div>
          ) : (
            filtered.map((msg) => (
              <ThreadRow
                key={msg.id}
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-white">
        {selectedMsg ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-8">
            ...
            <p className="text-slate-600 font-medium">Select a message</p>
            <p className="text-slate-400 text-sm mt-1">
              Choose from the list or send a new message.
            </p>
            <button
              type="button"
              onClick={() => router.push("/compose")}
